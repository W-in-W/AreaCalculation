 ...
 * Проверка на то, является ли треугольник прямоугольным выполняется путём вызова метода IsRightTriangle() на
 * объекте типа Triangle.
 * Вычисление площади фигуры неизвестного типа происходит путём создания объекта класса UnknownFigure и
 * вызова у него метода GetArea().
